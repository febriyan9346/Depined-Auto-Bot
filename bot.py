...
"""
    ...
    def process_account_first_cycle(self, account_line: str, account_num: int, total_accounts: int, proxy: Optional[str] = None):
    ...
    def process_account_next_cycle(self, account_line: str, account_num: int, total_accounts: int, proxy: Optional[str] = None):
        self.log(f"Account #{account_num}/{total_accounts}", "INFO")
        ...
        if not token:
            self.log("Token not found, skipping...", "ERROR")
            return False
    ...
    def run(self):
        ...
        while True:
            ...
            for idx, account_line in enumerate(self.accounts, 1):
                ...
                if cycle == 1:
                    if self.process_account_first_cycle(account_line, idx, total_accounts, proxy):
                        success_count += 1
                else:
                    if self.process_account_next_cycle(account_line, idx, total_accounts, proxy):
                        success_count += 1
